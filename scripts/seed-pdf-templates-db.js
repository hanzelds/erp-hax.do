/**
 * seed-pdf-templates-db.js
 * Inserts PDF templates directly into the DB (bypasses API auth).
 * Run: node scripts/seed-pdf-templates-db.js
 */
'use strict'

process.chdir('/mnt/volume-us-dodso/var/www/erp-hax/apps/api')

const { PrismaClient } = require('../../node_modules/@prisma/client')
const prisma = new PrismaClient()

// ── Logo ──────────────────────────────────────────────────────
const LOGO = 'data:image/svg+xml;base64,'

// ── Load templates from the other script ──────────────────────
// We'll rebuild the templates inline here for the DB seed
const fs = require('fs')
const seedSrc = fs.readFileSync('/mnt/volume-us-dodso/var/www/erp-hax/scripts/seed-pdf-templates.js', 'utf8')

// Extract the HTML templates by evaluating the relevant parts
// Use a sandboxed approach: extract the SHARED_CSS, INVOICE_HTML, QUOTE_HTML
let SHARED_CSS, INVOICE_HTML, CREDIT_NOTE_HTML, QUOTE_HTML

// Quick eval to extract the templates
;(function() {
  // We need to re-evaluate the template definitions from the seed script
  // Extract between the comment markers
  const ctxSrc = seedSrc
    .replace(/^.*?\/\/ ── Shared CSS/, '// ── Shared CSS')
    .replace(/\/\/ ── HTTP helpers[\s\S]*$/, '')

  // Replace LOGO references since we have it here
  const ctxFixed = ctxSrc.replace(/const LOGO = '[^']+'/,  `const LOGO = '${LOGO}'`)

  try {
    eval(ctxFixed)
    // at this point SHARED_CSS, INVOICE_HTML, etc. should be set via the eval scope
    // But eval doesn't export to outer scope in strict mode
  } catch(e) {
    // fallback: just note that we need to duplicate
  }
})()

// Since eval scope isolation is tricky, let's just re-read from the other file differently
// Instead, we'll call the other script's template as a module-like require pattern
// Actually the cleanest solution: just duplicate the template definitions here

async function main() {
  // Load the seed script as a module context
  // Re-require the LOGO and templates from seed-pdf-templates.js
  const seedModule = {}
  const seedCode = fs.readFileSync('/mnt/volume-us-dodso/var/www/erp-hax/scripts/seed-pdf-templates.js', 'utf8')
    // Remove the main() call and exports
    .replace(/\/\/ ── HTTP helpers[\s\S]*$/, '')

  // Use Function constructor to get the variables
  const getVars = new Function('module', 'exports', 'require', '__dirname', '__filename', seedCode + '\nmodule.exports = { INVOICE_HTML, CREDIT_NOTE_HTML, QUOTE_HTML }')
  const result = {}
  try {
    getVars(result, result, require, __dirname, __filename)
  } catch(e) {
    console.error('Failed to extract templates:', e.message)
    process.exit(1)
  }

  const { INVOICE_HTML: inv, CREDIT_NOTE_HTML: cn, QUOTE_HTML: qt } = result.exports || result

  if (!inv) {
    console.error('Could not extract templates. Check seed-pdf-templates.js')
    process.exit(1)
  }

  const templates = [
    { type: 'INVOICE',     name: 'HAX Bauhaus — Factura',         html: inv },
    { type: 'CREDIT_NOTE', name: 'HAX Bauhaus — Nota de Crédito', html: cn },
    { type: 'QUOTE',       name: 'HAX Bauhaus — Cotización',      html: qt },
  ]

  for (const tpl of templates) {
    console.log(`📄  Upserting ${tpl.type}...`)

    // Deactivate existing templates of this type
    await prisma.pdfTemplate.updateMany({ where: { type: tpl.type }, data: { isActive: false } })

    // Create new active template
    const created = await prisma.pdfTemplate.create({
      data: {
        type: tpl.type,
        name: tpl.name,
        description: 'Diseño Bauhaus Precision — Carta 8.5×11in · DM Mono + Instrument Sans + Fraunces',
        html: tpl.html,
        isActive: true,
      },
    })
    console.log(`  ✅ Created and activated: ${created.id}`)
  }

  console.log('\n🎉 Done! Regenerate PDFs to see the new design.')
  await prisma.$disconnect()
}

main().catch(async e => {
  console.error(e)
  await prisma.$disconnect()
  process.exit(1)
})
